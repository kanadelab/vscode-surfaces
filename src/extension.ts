...
class SurfacesKeywordDesc {
	...
	public constructor(name: string, detail: string, shortDetail: string, document: string | null = null, primary: boolean = false) {
		this.name = name;
		this.detail = detail;
...
class SurfacesSignatureHelpProvider implements vscode.SignatureHelpProvider {

	private selectSignature(signature: vscode.SignatureInformation, parameterIndex: number): vscode.SignatureHelp {
	...
	private selectSignatureToSplitItemCount(signature: vscode.SignatureInformation, splitItemsLength: number, lastWideRange: boolean = false): vscode.SignatureHelp | null {
...
class SurfacesCompletionItemProvider implements vscode.CompletionItemProvider {
	...
	private MakeIntervalItems(): vscode.CompletionItem[] {
		...
		for (var i of items) {
			var ci = new vscode.CompletionItem(i[0] as string);

			if (i[0] == "sometimes") {
				ci.preselect = true;
			}

			ci.detail = i[1] as string;
	...
	private MakeMergeMethodItems(): vscode.CompletionItem[] {
		var result: vscode.CompletionItem[] = [];
	...
	private MakeAnimationItems(): vscode.CompletionItem[] {
		var result: vscode.CompletionItem[] = [];
	...
	private MakeCollisionItems(): vscode.CompletionItem[] {
		var result: vscode.CompletionItem[] = [];
...
class SurfacesDefinitionProvider implements vscode.DefinitionProvider {
	...
	provideDefinition(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): vscode.ProviderResult<vscode.Definition | vscode.LocationLink[]> {
		...
		//検索
		var resultLocations: vscode.Location[] = [];
